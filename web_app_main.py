...
def fileUpload(init_file) :
    st.subheader("ファイル読み込み")
    # アップロードファイルをメインにデータ表示
    df_init = web_app_graph.readInitFile(init_file)
    ...
    if choice1 == "ホーム" :
        ...
        if st.sidebar.checkbox("ログイン") :
            ...
            if result :
                ...
                df_init, df_data = fileUpload("init//init.csv")
                ...
                with st.form(key='show_graph_form') :
                    ...
                    # 詳細設定
                    fig_list_xlabel, fig_list_ylabel, fig_number = web_app_graph.drawPlot(df_data, ax1, ax2, second_axis)
                    #軸設定（対数、最小大、目盛り、ラベル）
                    fig_type = web_app_graph.selectFigType(ax1)
                    web_app_graph.graidShow(ax1)
                    web_app_graph.axisSetting(df_data, fig_type, ax1, ax2, second_axis, fig_list_xlabel, fig_list_ylabel)
